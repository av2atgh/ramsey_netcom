"""Build cumulative cond-mat co-authorship networks from the arXiv metadata dump.

Reads the Kaggle arXiv metadata snapshot (JSON-lines), keeps papers in the
cond-mat archive, and builds the co-authorship graph (an edge between every pair
of co-authors of a paper). One *cumulative* network is written per year Y -- it
contains all authors and co-authorships of cond-mat papers submitted up to and
including Y -- saved as examples/data/condmat_coauthor_upto_<Y>.gt.gz, plus an
index CSV with (year, n_nodes, n_edges).
...
Set CONDMAT_LIMIT=<k> to stop after k cond-mat papers (quick test).

Run from anywhere (large one-time read of the ~5 GB dump):
    python examples/build_condmat_coauthor.py
"""
import json
import os
import re
...
INDEX_CSV = os.path.join(DATA_DIR, "condmat_coauthor_index.csv")
CATEGORY = "cond-mat"  # match this archive (any "cond-mat" or "cond-mat.*" tag)
...
def read_condmat_papers():
    ...
    with open(JSON_PATH, "rb") as f:
        for raw in f:
            if b"cond-mat" not in raw:  # cheap pre-filter before JSON parse
                continue
            rec = json.loads(raw)
        ...
        for ids in by_year[year]:
            ...
            for a, b in combinations(sorted(set(vs)), 2):
                if (a, b) not in edge_set:
                    edge_set.add((a, b))
                    g.add_edge(a, b)
        out = os.path.join(DATA_DIR, f"condmat_coauthor_upto_{year}.gt.gz")
        g.save(out)
...
def main():
    ...
    papers, id_name = read_condmat_papers()
    print(f"cond-mat papers: {len(papers)}  unique authors: {len(id_name)}")
